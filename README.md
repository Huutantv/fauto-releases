...
```bash
fauto config     # base URL → API key → pick a model
fauto            # land in the REPL and start typing
```

That's the whole setup. No env vars, no config file to hand-edit.

<sub>Prefer to do it by hand? Grab a binary from the
[latest release](https://github.com/Huutantv/factory-ai/releases/latest) — or build it yourself with
`cargo install --git https://github.com/Huutantv/factory-ai`. Upgrade or roll back any time with
...
| **It runs where you aren't** | `fauto serve` drives the agent from Telegram or Discord and asks your phone to approve risky edits. Host it on systemd, Docker, or Kubernetes — behind NAT, no inbound port. |
| **Safe by construction** | An OS sandbox under the approval layer: children never inherit your API keys, network is deny-by-default, and the filesystem policy is kernel-enforced on Linux (Landlock+seccomp) and macOS (Seatbelt) — Windows gets Job-Object containment and honest `partial` reporting, not pretend enforcement. A hard command floor refuses catastrophic commands **even under auto-approve**. `fauto sandbox status` shows exactly what *your* machine enforces — see [docs/SANDBOX.md](docs/SANDBOX.md). |

## What it can do

```
  fauto agent "fix the failing parse test"
...
| **Multi-agent** | `fauto workflow` fans out role-scoped sub-agents and synthesises one answer. |
...
"F.Auto" and the logo are trademarks of the F.Auto authors; §6 grants no trademark rights, so a fork
must not present itself as F.Auto. Releases up to v0.5.5 shipped under PolyForm Noncommercial;
